...
Forge manages guest agent files in project `agents/` directories. These are lightweight persona definitions (markdown files with YAML frontmatter) that the Anthem orchestrator dispatches as independent specialists.

Full implementation spec: `docs/guest-agents.md`

### Shipped
- `scaffold_agents_directory()`: creates `agents/` dir with starter agents during `scaffold_project` (game, web, api presets)
